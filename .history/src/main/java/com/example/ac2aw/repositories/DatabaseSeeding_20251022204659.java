...
public class DatabaseSeeding implements CommandLineRunner {
    ...
    // Use CommandLineRunner to execute logic on application startup
    @Override
    @Transactional // Ensure all operations are within a single transaction
    public void run(String... args) throws Exception {
        // Clear existing data to ensure idempotent seeding (optional, but good for testing)
        // Note: You might need to adjust the order or use TRUNCATE for production-like scenarios
        // For simplicity, we'll rely on the database dropping/recreating based on spring.jpa.hibernate.ddl-auto property
        
        // Ensure the tables are empty before seeding (optional check)
        ...
        funcionarioRepository.saveAll(funcionarios); // Relationships are managed on the owning side (Funcionario for Setor)
        ...
        // Since Funcionario is the owning side in the ManyToMany (Funcionario has mappedBy="funcionarios" and Projeto has mappedBy="projetos"),
        // the relationship is defined by the Funcionario entity's persistence,
        // but the 'projetos' list in Projeto needs to be updated if you want to access it from the Projeto side.
        // However, looking at your mappings:
        // Funcionario has: @ManyToMany(mappedBy = "funcionarios") @JoinColumn(name = "projeto_id") private Projeto projeto; <-- This seems incorrect for ManyToMany, should be List<Projeto>
        // Projeto has: @ManyToMany (mappedBy = "projetos") @JoinColumn(name = "funcionario_id") private List<Funcionario> funcionarios; <-- This is the owning side in this config!
        
        // **Correction based on standard JPA practice:**
        // The entity without `mappedBy` (or with `@JoinTable`) is usually the **owning side**.
        // In your provided code, the relationship mapping is unconventional and likely incorrect:
        // - Funcionario: Has `mappedBy = "funcionarios"` but also `@JoinColumn(name = "projeto_id")` and the type is singular `Projeto`. This looks like a mix-up between ManyToOne and ManyToMany.
        // - Projeto: Has `mappedBy = "projetos"` and the type is `List<Funcionario>`. This would typically mean Projeto is the **inverse side**.
        
        // Assuming the intent was that **Funcionario is the Owning Side** of the Many-to-Many relationship (which is good practice for the *many* side):
        // 1. **Funcionario** should define the `@ManyToMany` with `@JoinTable` (or `@JoinColumn` on the intermediate table if Hibernate DDL is used)
        //    *AND* its attribute should be `List<Projeto>`.
        // 2. **Projeto** should define the `@ManyToMany(mappedBy = "projetos")` and its attribute should be `List<Funcionario>`.
        
        // Since I must use your entities as is, I will **assume** the application is configured such that the saving of `Funcionario` *correctly* establishes the link in the join table, even with the unconventional singular `Projeto projeto;` and `mappedBy` on the `Funcionario` side.
        // The following assumes the relationships are correctly set by saving the Funcionario objects *with* their associated Setor and Projeto/s.
    ...
    private List<Projeto> createProjetos() {
        ...
        projetos.add(new Projeto("New CRM Implementation", LocalDate.now().minusMonths(3), LocalDate.now().plusMonths(9)));
    ...
    private List<Funcionario> createFuncionarios(List<Setor> setores, List<Projeto> projetos) {
        ...
        // If a true Many-to-Many was intended, Funcionario should have `List<Projeto> projetos`, 
