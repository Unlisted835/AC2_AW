...
public class DatabaseSeeding implements Runnable {
   ...
   private List<Funcionario> createFuncionarios(List<Setor> setores, List<Projeto> projetos) {
      ...
      // Manually update the inverse side (Projeto) for bidirectional consistency
      // This step is highly recommended for Many-to-Many relationships
      for (Funcionario f : funcionarios) {
         if (f.getProjetos() != null) {
            for (Projeto p : f.getProjetos()) {
               if (p.getFuncionarios() == null) {
                  p.setFuncionarios(new ArrayList<>());
               }
               if (!p.getFuncionarios().contains(f)) {
      ...
      // Re-save projects to ensure the inverse side is updated (depends on JPA
      // provider settings)
      // Since Funcionario is the assumed owning side, saving Funcionario *should*
      // handle the join table,
      // but explicitly updating the entity lists maintains object consistency.
