...
public class FuncionarioController {
   ...
   public Funcionario salvar(@RequestBody FuncionarioCreateDTO funcionarioDTO) {
      Funcionario f = service.save(funcionarioDTO);
      return f;
   }

   @DeleteMapping("{id}")
   @ResponseStatus(HttpStatus.NO_CONTENT)
   public void funcionarioCurso(@PathVariable Long id) {
      cursoService.remover(id);
   }

   @PutMapping("{id}")
   public void editFuncionario(@PathVariable Long id, @RequestBody FuncionarioDTO dto) {
      funcionarioService.editar(id, dto);
   }
