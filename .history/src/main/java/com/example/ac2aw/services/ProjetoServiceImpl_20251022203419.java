...
public class ProjetoServiceImpl implements ProjetoService {
   ...
   public void assignFuncionarioToProjeto(int projetoId, int funcionarioId) {
      ...
      boolean alreadyAssigned = projeto.getFuncionarios().stream()
         .anyMatch(funcionario -> funcionario.getId() == funcionarioId);

      if (alreadyAssigned) {
         throw new ValidationException("Funcionário já está atribuído ao projeto.");
      }

      repository.assignFuncionarioToProjeto(projetoId, funcionarioId);
   }
